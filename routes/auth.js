...
router.post('/login', async (req, res) => {
  ...
  const emailExist = await User.findOne({ email: req.body.email });
  if (!emailExist)
    return res.status(400).send('Логин или пароль введен не верно.');
  ...
  if (!passExist)
    return res.status(400).send('Пароль введен не верно.');
...
router.get('/auth/me', verifySession, async (req, res) => {
  ...
  MongoClient.connect(
    ...
    function (err, db) {
      ...
      dbo
        .collection('users')
        .findOne({ uid: userId }, function (errorr, result) {
  ...
  if (req.session.userId) {
    req.session.destroy(function (err) {
      res.status(200).clearCookie('connect.sid').send({
        apiData: {},
        resultCode: 0,
        message: 'Вы вышли из профиля.(foo)',
      });
    });
